...
def read_traj_dataset(file_path):
    ...
    _time = time.time()
    trajs = pd.read_pickle(
        file_path,
    )

    l = trajs.shape[0]
    # print(l)
    # origin: 200000 modify it to 50000
    train_idx = (int(l * 0), 50000)
    ...
    logging.info(
        "[Load traj dataset] END. @={:.0f}, #={}({}/{}/{})".format(
            time.time() - _time, l, len(_train), len(_eval), len(_test)
        )
    )
...
def read_trajsimi_traj_dataset(file_path):
    ...
    l = df_trajs.shape[0]
    # original 10000, 5==num_cls, length added to 30000
    ...
    logging.info(
        "trajsimi traj dataset sizes. traj: #total={} (trains/evals/tests={}/{}/{})".format(
            l, trains.shape[0], evals.shape[0], tests.shape[0]
        )
    )
...
def read_trajsimi_simi_dataset(file_path):
    ...
    with open(file_path, "rb") as fh:
        ...
        logging.info(
            "[trajsimi simi dataset loaded] @={}, trains/evals/tests={}/{}/{}".format(
                time.time() - _time, len(trains_simi), len(evals_simi), len(tests_simi)
            )
        )
